import "./IFractionalExponents.sol";
...
contract FractionalExponents is IFractionalExponents {
  ...
  function _bancorFormula() internal {
  ...
  */
  function power(uint256 _baseN, uint256 _baseD, uint32 _expN, uint32 _expD) external view override returns (uint256, uint8) {
      assert(_baseN < MAX_NUM);

      if (_expN == 0) {
          return (FIXED_1, MIN_PRECISION);
      } else if (_baseN == 0) {
          return (0, MIN_PRECISION);
      }
