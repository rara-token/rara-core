...
contract EIP210 {
    ...
    // @dev returns whether the block hash at the given block was simulated,
    // potentially recording it as well. If `false` the hash will match that
    // reported by off-chain tools.
    function eip210BlockhashSimulated(uint256 _blockNumber) external returns (bool _simulated) {
        bytes32 _blockhash = _hash[_blockNumber];
        if (_blockhash == 0) {
            (, _simulated) = _write(_blockNumber);
        } else {
            _simulated = _hashSimulated[_blockNumber];
        }
    }

    // @dev returns the block hash at the given block, but without storing the
    // result if not already stored; i.e. this is a view function and does not
    // write to the chain. As a result, it's possible that the value returned
    // will change over time (every 256 blocks). Check `_immutable` for assurance
    // that the output will not change.
    function eip210BlockhashEstimate(uint256 _blockNumber) external view returns (bytes32 _blockhash, bool _immutable) {
        _blockhash = _hash[_blockNumber];
        if (_blockhash == 0) {
          (_blockhash,) = _calculate(_blockNumber);
        } else {
          _immutable = true;
        }
    }
    ...
    function _write(uint256 _blockNumber) internal returns (bytes32 _blockhash, bool _simulated) {
          (_blockhash, _simulated) = _calculate(_blockNumber);
          if (_blockhash != 0) {
              _hash[_blockNumber] = _blockhash;
              _hashSimulated[_blockNumber] = _simulated;
          }
    }

    function _calculate(uint256 _blockNumber) internal view returns (bytes32 _blockhash, bool _simulated) {
      if (_blockNumber < block.number) {  // block occurred in the past; ok to calculate
