...
import "../../utils/lending/ILendingPool.sol";
...
contract WithdrawFeeValidatingStakeManager is IStakeManager, LendingPool {
    ...
    /// @notice an ILendingPool we draw funds from if needed
    ...
    mapping (uint256 => bool) stakeMigrated;

    uint256 private constant PRECISION = 1e20;

    constructor(address _pool, address _lender, uint256 _feeNumerator, uint256 _feeDenominator, uint256 _feePeriod, address _feeRecipient) {
        require(_feeDenominator > 0, "WithdrawFeeValidatingStakeManager: feeDenominator must be non-zero");
        require(_feeNumerator <= _feeDenominator, "WithdrawFeeValidatingStakeManager: fee numerator must be <= denominator");
        require(_feeNumerator < PRECISION && _feeDenominator < PRECISION, "WithdrawFeeValidatingStakeManager: fee precision too high");
    ...
    function setWithdrawFee(uint256 _feeNumerator, uint256 _feeDenominator, uint256 _feePeriod, address _feeRecipient, bool overwrite) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "WithdrawFeeValidatingStakeManager: must have MANAGER role to setWithdrawFee");
        require(_feeDenominator > 0, "WithdrawFeeValidatingStakeManager: feeDenominator must be non-zero");
        require(_feeNumerator <= _feeDenominator, "WithdrawFeeValidatingStakeManager: fee numerator must be <= denominator");
        require(_feeNumerator < PRECISION && _feeDenominator < PRECISION, "WithdrawFeeValidatingStakeManager: fee precision too high");
    ...
    function onStakeDeposit(uint256 pid, IERC20 stakeToken, address user, address recipient, uint256 deposit, uint256) external override returns (bool) {
        require(_msgSender() == pool, "WithdrawFeeValidatingStakeManager: only mining pool can onStakeDeposit");
        require(user == recipient, "WithdrawFeeValidatingStakeManager: stake funder must be == recipient");
        require(deposit > 0, "WithdrawFeeValidatingStakeManager: must deposit > 0");
    ...
    function onStakeWithdraw(uint256 pid, IERC20 stakeToken, address user, address recipient, uint256 withdrawal, uint256) external override returns (bool) {
        require(_msgSender() == pool, "WithdrawFeeValidatingStakeManager: only mining pool can onStakeWithdraw");
        require(withdrawal > 0, "WithdrawFeeValidatingStakeManager: must withdraw > 0");
        if (!stakeMigrated[pid] && lender != address(0)) {
            ILendingPool(lender).lend(stakeToken, address(this), stakeToken.balanceOf(lender));
            stakeMigrated[pid] = true;
        }
    ...
    function onRewardHarvest(uint256, IERC20, address, address, uint256 rewardAmount, uint256) external override returns (bool handled) {
        require(rewardAmount <= IMiningPool(pool).totalMined(), "WithdrawFeeValidatingStakeManager: harvest underflow");
    }
}
