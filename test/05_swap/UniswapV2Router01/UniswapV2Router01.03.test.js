...
const { MAX_UINT256, ZERO_ADDRESS } = require('@openzeppelin/test-helpers').constants;
...
contract('UniswapV2Router02 (UniswapV2Router01 interface) with SwapValuator and discount fees', ([alice, bob, carol, minter, scrooge]) => {
  beforeEach(async () => {
    // deploy tokens
    const tokenA = await MockERC20.new('TOKEN_A', 'A', expandTo18Decimals(100000));
    const tokenB = await MockERC20.new('TOKEN_B', 'B', expandTo18Decimals(100000));
    this.WETH = await MockWETH.new();
    this.WETHPartner = await MockERC20.new('TOKEN_WP', 'WP', expandTo18Decimals(100000));
    ...
    this.valuator.setSwapRateDimi(this.routerEventEmitter.address, 10000);
  ...
  it('addLiquidity', async () => {
    ...
    await expectEvent.inTransaction(tx, pair, 'Transfer', { from:ZERO_ADDRESS, to:ZERO_ADDRESS, value:MINIMUM_LIQUIDITY });
    await expectEvent.inTransaction(tx, pair, 'Transfer', { from:ZERO_ADDRESS, to:wallet.address, value:expectedLiquidity.sub(MINIMUM_LIQUIDITY) });
    await expectEvent.inTransaction(tx, pair, 'Sync', { reserve0:token0Amount, reserve1:token1Amount });
    await expectEvent.inTransaction(tx, pair, 'Mint', { sender:router.address, amount0:token0Amount, amount1:token1Amount });
  ...
  it('addLiquidityETH', async () => {
    ...
    await WETHPartner.approve(router.address, MAX_UINT256, { from:scrooge });
    ...
    await expectEvent.inTransaction(tx, WETHPair, 'Transfer', { from:ZERO_ADDRESS, to:ZERO_ADDRESS, value:MINIMUM_LIQUIDITY });
    await expectEvent.inTransaction(tx, WETHPair, 'Transfer', { from:ZERO_ADDRESS, to:wallet.address, value:expectedLiquidity.sub(MINIMUM_LIQUIDITY) });
  ...
  it('removeLiquidity', async () => {
    ...
    await expectEvent.inTransaction(tx, pair, 'Transfer', { from:pair.address, to:ZERO_ADDRESS, value:expectedLiquidity.sub(MINIMUM_LIQUIDITY) });
  ...
  it('removeLiquidityETH', async () => {
    ...
    await expectEvent.inTransaction(tx, WETHPair, 'Transfer', { from:WETHPair.address, to:ZERO_ADDRESS, value:expectedLiquidity.sub(MINIMUM_LIQUIDITY) });
  ...
  */
  ...
  context('50% discount (9985 rate)', () => {
    context('swapExactTokensForTokens', () => {
      const token0Amount = expandTo18Decimals(5)
      const token1Amount = expandTo18Decimals(10)
      const swapAmount = expandTo18Decimals(1)
      const expectedOutputAmount = bn('1664582812369759106')
    ...
    context('swapTokensForExactTokens', () => {
      const token0Amount = expandTo18Decimals(5)
      const token1Amount = expandTo18Decimals(10)
      const expectedSwapAmount = bn('556390140766705614')
    ...
    context('swapExactETHForTokens', () => {
      ...
      it('happy path', async () => {
        ...
        const WETHPairToken0 = await WETHPair.token0()
        const { tx } = await router.swapExactETHForTokens(0, [WETH.address, WETHPartner.address], wallet.address, MAX_UINT256, {
            value: swapAmount
        });
  ...
  context('10% discount (9973 rate)', () => {
    beforeEach(async () => {
      await this.token0.transfer(bob, expandTo18Decimals(10000));
      await this.token1.transfer(bob, expandTo18Decimals(10000));
      await this.WETHPartner.transfer(bob, expandTo18Decimals(10000));
    });
    ...
    context('swapExactTokensForTokens', () => {
      ...
      const token1Amount = expandTo18Decimals(10)
      const swapAmount = expandTo18Decimals(1)
      const expectedOutputAmount = bn('1662914978406949794')

      beforeEach(async () => {
        const { router, token0 } = this;
        await addLiquidity(token0Amount, token1Amount)
        await token0.approve(router.address, MAX_UINT256, { from:bob })
      });
      ...
      it('happy path', async () => {
        ...
        await expectEvent.inTransaction(tx, token0, 'Transfer', { from:bob, to:pair.address, value:swapAmount });
        await expectEvent.inTransaction(tx, token1, 'Transfer', { from:pair.address, to:wallet.address, value:expectedOutputAmount });
        await expectEvent.inTransaction(tx, pair, 'Sync', { reserve0:token0Amount.add(swapAmount), reserve1:token1Amount.sub(expectedOutputAmount) });
        await expectEvent.inTransaction(tx, pair, 'Swap', {
          sender:router.address,
      ...
      it('amounts', async () => {
        const { router, token0, token1, pair, wallet, routerEventEmitter } = this;
        await token0.approve(routerEventEmitter.address, MAX_UINT256, { from:bob })
        const { tx } = await routerEventEmitter.swapExactTokensForTokens(
            router.address,
            swapAmount,
            0,
    ...
    context('swapTokensForExactTokens', () => {
      const token0Amount = expandTo18Decimals(5)
      const token1Amount = expandTo18Decimals(10)
      const expectedSwapAmount = bn('557059616520159988')
      const outputAmount = expandTo18Decimals(1)
      ...
      it('happy path', async () => {
        ...
        await expectEvent.inTransaction(tx, token0, 'Transfer', { from:bob, to:pair.address, value:expectedSwapAmount });
        await expectEvent.inTransaction(tx, token1, 'Transfer', { from:pair.address, to:wallet.address, value:outputAmount });
        await expectEvent.inTransaction(tx, pair, 'Sync', { reserve0:token0Amount.add(expectedSwapAmount), reserve1:token1Amount.sub(outputAmount) });
        await expectEvent.inTransaction(tx, pair, 'Swap', {
          sender:router.address,
    ...
    context('swapTokensForExactETH', () => {
      ...
      it('happy path', async () => {
        const { router, WETH, WETHPartner, WETHPair, wallet } = this;
        await WETHPartner.approve(router.address, MAX_UINT256, { from:bob })
        const WETHPairToken0 = await WETHPair.token0()
        const { tx } = await router.swapTokensForExactETH(
            outputAmount,
            MAX_UINT256,
        ...
        ;
        await expectEvent.inTransaction(tx, WETHPartner, 'Transfer', { from:bob, to:WETHPair.address, value:expectedSwapAmount });
        await expectEvent.inTransaction(tx, WETH, 'Transfer', { src:WETHPair.address, dst:router.address, wad:outputAmount });
        await expectEvent.inTransaction(tx, WETHPair, 'Sync', {
          reserve0:WETHPairToken0 === WETHPartner.address
              ? WETHPartnerAmount.add(expectedSwapAmount)
      ...
      it('amounts', async () => {
        const { router, WETH, WETHPartner, WETHPair, wallet, routerEventEmitter } = this;
        await WETHPartner.approve(routerEventEmitter.address, MAX_UINT256, { from:bob })
        const { tx } = await routerEventEmitter.swapTokensForExactETH(
            router.address,
            outputAmount,
            MAX_UINT256,
    ...
    context('swapExactTokensForETH', () => {
      ...
      it('happy path', async () => {
        ...
        ;
        await expectEvent.inTransaction(tx, WETHPartner, 'Transfer', { from:bob, to:WETHPair.address, value:swapAmount });
        await expectEvent.inTransaction(tx, WETH, 'Transfer', { src:WETHPair.address, dst:router.address, wad:expectedOutputAmount });
        await expectEvent.inTransaction(tx, WETHPair, 'Sync', {
          reserve0:WETHPairToken0 === WETHPartner.address
            ? WETHPartnerAmount.add(swapAmount)
