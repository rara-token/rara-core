...
contract('RaraMiningPool', ([alice, bob, carol, dave, edith, minter, manager, lender, dev]) => {
  const MANAGER_ROLE = web3.utils.soliditySha3('MANAGER_ROLE');
  const LENDER_ROLE = web3.utils.soliditySha3('LENDER_ROLE');

  beforeEach(async () => {
    this.token = await MockERC20.new("Token", "T", '1000000000', { from: alice });
    this.emitter = await MockTokenEmitter.new(this.token.address, { from: alice });
    this.pool = await RaraMiningPool.new(this.token.address, this.emitter.address, 0, { from: alice });
    await this.pool.grantRole(MANAGER_ROLE, manager, { from:alice });
    await this.pool.grantRole(LENDER_ROLE, lender, { from:alice });
    await this.token.transfer(this.emitter.address, '1000000000', { from: alice });
  });

  it('should set correct state variables', async () => {
    const token = await this.pool.rara();
    ...
    assert.equal((await this.pool.burnNumerator()).valueOf().toString(), '5');
    ...
    assert.equal((await this.pool.hasRole(LENDER_ROLE, lender)).toString(), 'true');
  });
  ...
  context('With LP token added to the field', () => {
    ...
    it('should revert if non-manager adds a pool', async () => {
      await expectRevert.unspecified(this.pool.add('100', this.lp.address, ZERO_ADDRESS, { from:bob }));
